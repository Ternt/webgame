...
if not "%msvc%"=="1" if not "%clang%"=="1" if not "%wasm%"=="1" set msvc=1
if "%clang%"=="1" set msvc=0 && set wasm=0 && echo [compile clang]
if "%msvc%"=="1"  set clang=0 && set wasm=0 && echo [compile msvc]
if "%wasm%"=="1"  set clang=0 && set msvc=0 && echo [compile wasm]
...
pushd build
pushd bin
if "%game%"=="1"      if "%wasm%"=="1" set didbuild=1 && %compile% %root%code/game/game.c      %out%game.wasm || exit /b 1
if "%game%"=="1"                       set didbuild=1 && %compile% %root%code/game/game.c      %out%game.wasm || exit /b 1
if "%test_wasm%"=="1" if "%wasm%"=="1" set didbuild=1 && %compile% %root%code/test/test_wasm.c %out%test_wasm.wasm || exit /b 1
...
if "%build_web%"=="1" (
  pushd build
    ...
    popd web
  popd build
  cp code/web/*.js build/web/static/
  cp code/web/*.css build/web/static/
  cp code/web/*.html build/web/
